...
def main():
    ...
    ADMON_PASSWORD = env.str("ADMON_PASSWORD")
    ...
    def dell_spot_step_1(message):
        if str(message.chat.id) in ADMON_PASSWORD:
            mes = bot.send_message(message.chat.id, "Введите название горки, которую собираетесь удвлить", parse_mode='html')
